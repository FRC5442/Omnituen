...
public class SwerveMath {
    ...
    public Double[][] findModSpeedsHeadings(Vector2d moveVector, double rotation) {
        // variable names come from the white paper
        double a = moveVector.x - (rotation * (Robot.sharedConstants.RobotLength / 2));
        double b = moveVector.x + (rotation * (Robot.sharedConstants.RobotLength / 2));
        double c = moveVector.y - (rotation * (Robot.sharedConstants.RobotWidth / 2));
        double d = moveVector.y + (rotation * (Robot.sharedConstants.RobotWidth / 2));
